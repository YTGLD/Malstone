...
public class CorpseCauldron extends Twisted {
            ...
            if (Handler.hascurio(player, ItemRegs.CorpseCauldron_.get())) {
                ...
                if (foodProperties != null) {
                    if (compoundTag.getInt(foodData) < 150) {
                        int food = (int) (foodProperties.nutrition() + foodProperties.saturation());
                        ...
                        for (Map.Entry<String, ICurioStacksHandler> entry : curios.entrySet()) {
                            ...
                            for (int i = 0; i < stacksHandler.getSlots(); i++) {
                                ...
                                if (cc.is(ItemRegs.CorpseCauldron_.get())) {
                                    CompoundTag caTag = cc.get(DataReg.tag);
                                    if (caTag != null) {
    ...
    public Multimap<Holder<Attribute>, AttributeModifier> doAttribute(Player player, ItemStack stack) {
        ...
        modifiers.put(AttReg.ToughBlood, new AttributeModifier(ResourceLocation.fromNamespaceAndPath(Malstone.MODID,this.getDescriptionId() + "2"),
                it, AttributeModifier.Operation.ADD_VALUE));

        if (this.hasWeepingWllPower(stack)) {
            modifiers.put(AttReg.MagicRes, new AttributeModifier(ResourceLocation.fromNamespaceAndPath(Malstone.MODID,this.getDescriptionId()),
                    add, AttributeModifier.Operation.ADD_VALUE));
            modifiers.put(AttReg.DamageRes, new AttributeModifier(ResourceLocation.fromNamespaceAndPath(Malstone.MODID,this.getDescriptionId()),
                    add, AttributeModifier.Operation.ADD_VALUE));
            modifiers.put(AttReg.ToughBlood, new AttributeModifier(ResourceLocation.fromNamespaceAndPath(Malstone.MODID,this.getDescriptionId()),
                    add, AttributeModifier.Operation.ADD_MULTIPLIED_BASE));
            modifiers.put(AttReg.EatTime, new AttributeModifier(ResourceLocation.fromNamespaceAndPath(Malstone.MODID,this.getDescriptionId()),
                    -add * 2, AttributeModifier.Operation.ADD_MULTIPLIED_BASE));
        }
