...
public class ExtremelyDead extends Twisted {
                                ...
                                if (stack.is(ItemRegs.ExtremelyDead_.get())) {
                                    if (stack.getItem() instanceof BaseItem item) {
                                        if (item.canUseWeepingPower() && item.hasWeepingWllPower(stack)) {
                                            Vec3 vec3 = event.getEntity().position().add(0, 1, 0);
                                            SpiritItemEntity entity = new SpiritItemEntity(player.level(), player.getUUID(),
                                                    new ItemStack(ItemRegistry.UMBRAL_SPIRIT.get()),
                                                    vec3.x,vec3.y,vec3.z, 0,0,0);
    ...
    public Multimap<Attribute, AttributeModifier> doAttribute(ItemStack stack, LivingEntity entity) {
        ...
        modifiers.put(AttributeRegistry.SPIRIT_SPOILS.get(),new AttributeModifier(UUID.fromString("e479674e-ef3e-3736-9f9e-94f79aafe9d7"), this.getDescriptionId(),
                        spirit, AttributeModifier.Operation.ADDITION));
        modifiers.put(LodestoneAttributeRegistry.MAGIC_DAMAGE.get(),new AttributeModifier(UUID.fromString("e479674e-ef3e-3736-9f9e-94f79aafe9d7"), this.getDescriptionId(),
